...
受信・保存・配信は単一の .NET プロセスで行い、SQLite を保存済み状態の正本とします。画面は DB を直接参照せず、アプリケーションサーバーを経由します。Hub とアプリケーションサーバーの間にモックの合成点は置かず、検証時は接続設定の URL を制御可能な SSE サーバーへ向けます。画面とアプリケーションサーバーの間の合成点は、画面側で閲覧用APIを呼ぶ関数1箇所とします（[UCP-2](design/UCP-2.md)）。
