...
| --- | --- | --- |
| 起動・終了管理 | 接続設定とDBを準備し、全HubをDBへ受信中として登録してから受信を開始する。終了時は全Hubの受信を止め、処理中の保存が確定してからDBを閉じる | `backend/Hosting/AppHost.cs`、`backend/Features/HubSync/HubReceivers.cs` |
