...
// Personal で同期が保存を確定した後の状態。今日の利用が 250,000 トークン進み、5h 枠の残量が減る。
function advanced(): Overview {
  const next = structuredClone(initial);
  const hub = next.hubs.find((item) => item.hubId === 'personal')!;
  ...
  for (const period of Object.values(next.periods)) {
    ...
    const usage = period.hubs.find((item) => item.hubId === 'personal')!;
  ...
  next.limitWindows.find(
    (item) => item.accountKey === 'a1' && item.limitKey === 'five_hour',
  )!.remainingPercent = 55;
  next.devices.find((item) => item.deviceId === 'd1')!.updatedAt = '2026-09-25T03:17:30Z';
  return next;
}

export const overviewMocks: Overview[] = [initial, advanced()];
